...
本目录是 AI 知识库问答平台的 Spring Boot 后端子项目。迁移到 monorepo 后，后端相关命令请在 `backend/` 目录下执行：
...
Flyway 脚本仍位于 `src/main/resources/db/migration`，应用配置仍位于 `src/main/resources/application*.yml`。
